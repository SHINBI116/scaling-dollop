...
public class C07_Polymorphism {
	/*
	    ...
	    - 자식 클래스 인스턴스가 업캐스팅 된 상태에서
	 	...
	 	- 업캐스팅되었던 자식 클래스의 인스턴스만이 다시 원래의 타입으로 돌아올 수 있다
	 	  (원래 강아지 였던 인스턴스만 다시 강아지로 돌아올 수 잇다.
	...
	public static void main(String[] args) {
		...
		// Dog타입에만 존재하는 메서드인 walk()를 사용할 수 없다
		// 하지만 Wolf타입에도 함께 존재하는 메서드는 사용할 수 있다.
		Dog dog1 = new Dog();
		Wolf dog2 = new Dog("잠시늑대가된개");
		
		dog1.walk();
		//dog2.walk(); // 업캐스팅 되어서 사용 X
		
		dog1.pet();
		dog2.pet(); //오버라이딩된 메서드는 업캐스팅되더라도 자식클래스의 것으로 동작한다
