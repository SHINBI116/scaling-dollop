...
//		CopyDirectory dir2 = new CopyDirectory("./myimages/animals/dogs");
//		
...
class CopyDirectory {
	...
	private void copyFile(File f) {
		...
		// 복사하려는 경로의 부모폴더가 아직 없다면 폴더를 생성한다
	...
	// 텍스트 파일 복사에는 문제가 없지만 읽을 때 그림 데이터를 문자로 바꿔버려서 그림이 깨진다
	...
	private void copyFolder(File dir) {
		...
		for (File f : dir.listFiles()) {
			...
			// .\myimages\memo2\test2.txt
			// \memo\test2.txt
//			System.out.println("-------------------");
//			System.out.println("name : " + f.getName());
//			System.out.println("path: " + f.getPath());
//			System.out.println("sub: " + f.getPath().substring(srcPath.length()));
//			System.out.println("add Patrent: " + 
//			new File(destPath, f.getPath().substring(srcPath.length())));
		}
	}
