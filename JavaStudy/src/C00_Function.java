...
public class C00_Function {
	...
	//int... : 해당 타입을 1개 이상 받을 수 있지만 배열타입으로 받는다
	public static void plusAll(int ... num) {
		int sum = 0;
		for (int i = 0; i < num.length; ++i) {
			sum += num[i];
		}
		System.out.println("전달한 모든 숫자 합 " + sum);
	}
...
//	}
	...
	// void 이 함수는 호출해봤자 아무값도 돌려 받지 못합니다
	...
	//		 필요한 바구니의 개수를 리턴해주는 함수
	
	public static int getBasketCount (int appleCount, int bucketSize) {
		int basketCount;
		if (appleCount < 0 || bucketSize < 0) {
			return -1;
		}
		
		return appleCount % bucketSize == 0 ?
				appleCount / bucketSize : appleCount / bucketSize + 1;
//		if (appleCount % bucketSize == 0) {
//			return basketCount = appleCount / bucketSize;	
//		} else {
//			return basketCount = appleCount / bucketSize + 1;
//		}
	}
	
	public static int getBasketCount2 (int appleCount, int bucketSize) {
		int cnt = 0;
		if (appleCount < 0 || bucketSize < 0) {
			return -1;
		}
		while (appleCount > 0) {
			appleCount -= bucketSize;
			++cnt;
		}
		return cnt;
	}
	...
	public static void main(String[] args) {
		System.out.println("바구니의 개수 : " + getBasketCount(-27, 3));
