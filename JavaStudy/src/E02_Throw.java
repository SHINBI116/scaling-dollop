...
public class E02_Throw {
	/*
	 	...
	 	- RuntimeException을 상속받는 경우 굳이 throws로 알려주지 않아도 되는 사소한 예외가된다
	 	- Exception을 상속받은 경우 반드시 예외처리가 되어야 컴파일이 가능한 중요한 예외가 된다
	 	  이 경우 throws로 반드시 알려줘야하는 예외가 된다
	 */
	
	public static void method1(int a) throws InputMismatchException, NotEnoughMoneyException {
	...
	public static void method2() throws TooFastSpeedException {
		...
		if (ran == 0) {
			// Exception을 상속받은 예외는 반드시 throws로 추가되어야한다
