...
public class B00_Scanner {
	/*
	 	...
	 	-최초로는 문자열형태로 값을 꺼낸 후 원하는  타입으로 변환해준다
	 	-입력 통로의 값들은 공백,\t,\n을 기준으로 구분할 수 있다
	 	...
	 	-입력 통로의 값들은 공백,\t,\n 을 기준으로 구분하기 때문에 문장은 입력x
	 	
	 	(3) nextLine() 메서드
	 	
	 	-입력 통로로부터 다음 값 하나를 문자열로 꺼내는 기능
	 	-\n만을 기준으로 구분하기 때문에 한 줄 전체를 입력받을 수 잇다
	...
	public static void main(String[] args) {
		...
		//nextInt()를 쓰고 난 후에 남아있는 \n이 문제가 된다면
		//nextLine()을 사용해 \n을 비워버릴 수 있다
		String trash = sc.nextLine();
		//System.out.println("nextInt() 쓰고난 후 남은찌꺼기: "+trash);
