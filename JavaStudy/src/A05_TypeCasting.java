...
public class A05_TypeCasting {
	...
	/*
	 	...
	 	-크기가 더  큰 타입의 변수에 작은 크기의 값을 넣을때는 타입이 자동으로 변환한다.
	 	(자연스러운 타입 캐스팅)
	...
	public static void main(String[] args) {
		...
		//작은 타입 값이 큰 타입에 들어갈때는 별도의 조치 없이도 타입이 변한다
		byte b1 = 127;
		...
		//큰 타입 값을 작은 타입에 넣을 때는 강제 타입 캐스팅(강제형변환)으로 대입이 가능하지만
		//값이 손상될 수도 있다
		int num= 200;
		
		byte bnum = (byte) num;
		
		//System.out.println(bnum);//-56 값손상
		
		double d1 =123.1234565654456;
		float f1 = (float)d1;
		...
		short sss = -1;
		char ccc=(char)sss;
		System.out.println((int)ccc);//65535
		...
		//short에 char값 을 넣을때는 32767을 초과하는 양수가 존재할 위험이 있다
		//snum =(short) ch1;
