...
public class A04_VariableNAming {
	...
	/*
	  ...
	  3. 변수명 첫 글자는 알파벳 소문자를 사용해야한다
	    (첫 글자가 대문자인 경우는 암묵적으로 클래스로 간주)
	    
	    ex>Apple(클래스로 간주),apple(변수로 간주)
	    
	  4. 프로그램이 종료될때까지 값이 절대로 변하지 않을 값은 모두 대문자를 사용
	  (상수를 저장하는 변수명은 모두 대문자로 표기)
	  ...
	  6. 해당 변수에 들어있는 값이 무엇인지 윕게 파악할 수 있는 이름을 사용
